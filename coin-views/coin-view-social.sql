WITH CTE_PC AS (
    ...
    FROM
        PRD_BIWA_ONLINEMARKETINGSTEUERUNG.ONLINEMARKETINGSTEUERUNG_TRACKINGLEVEL_MASTERDATA
...
FROM (
    ...
    SELECT
        DAT.REPORTING_DATE
        , FIRST_VALUE(
            COALESCE(CTE_PC.CHANNEL, 'Social Ads')
        ) 
            AS CHANNEL
        , COALESCE(DAT.CAMP_ID, CTE_PC.CAMP_ID) AS CAMP_ID
        , FIRST_VALUE(
            COALESCE(
    ...
    LEFT JOIN
        CTE_PC
        ON DAT.FACEBOOK_ADSET_ID = CTE_PC.ADGROUP_ID
        AND DAT.FACEBOOK_AD_ID = CTE_PC.ADVERTISING_MEDIUM_ID
    WHERE
    ...
    GROUP BY
        DAT.REPORTING_DATE, LOCAL.CAMP_ID
    ...
    LEFT JOIN
        CTE_PC
        ON CAST(DAT.PINTEREST_ADGROUP_ID AS CHAR(100)) = CTE_PC.ADGROUP_ID
        AND CAST(DAT.PINTEREST_PROMOTION_PIN_ID AS CHAR(100)) = CTE_PC.ADVERTISING_MEDIUM_ID
    WHERE
        DAT.REPORTING_DATE >= '2020-01-01'
    ...
    LEFT JOIN
        CTE_PC
        ON DAT.SNAP_ADSQUAD_ID = CTE_PC.ADGROUP_ID
        AND DAT.SNAP_AD_ID = CTE_PC.ADVERTISING_MEDIUM_ID
    WHERE
        DAT.REPORTING_DATE >= '2020-01-01'
    ...
    SELECT
        ...
        , COALESCE(DAT.CAMP_ID, CTE_PC.CAMP_ID) AS CAMP_ID
        , FIRST_VALUE(
            COALESCE(CTE_PC.PLANNING_CONTEXT_CODE, 'Y.MP-GV.AS-SA.C')
        ) AS PLANNING_CONTEXT_CODE
        , FIRST_VALUE(
            COALESCE(CTE_PC.PLANNING_CONTEXT_NAME, 'YouTube - Google Video') 
        )
            AS PLANNING_CONTEXT_NAME
    ...
    LEFT JOIN
        CTE_PC
        ON CAST(DAT.GOOGLE_ADGROUP_ID AS CHAR(100)) = CTE_PC.ADGROUP_ID
        AND CAST(DAT.GOOGLE_AD_ID AS CHAR(100)) = CTE_PC.ADVERTISING_MEDIUM_ID
    WHERE
    ...
    LEFT JOIN
        CTE_PC
        ON DAT.TIKTOK_ADGROUP_ID = CTE_PC.ADGROUP_ID
        AND DAT.TIKTOK_AD_ID = CTE_PC.ADVERTISING_MEDIUM_ID
